...
class ProductFoodBook(BaseBook):
    ...
    def append(self, product_food, group_number, id_in_group=None):
        ...
        self._foods_by_group[group_number][id_in_group] = product_food
        # 採番した総合 ID を返す
    ...
    def update(self, total_id, entry):
        ...
        if id_in_group not in self._foods_by_group[group_number]:
            raise ValueError('Not found id in the group. group={}, id_in_group={}'.format(group_number, id_in_group))

        orig = self._foods_by_group[group_number][id_in_group]
    ...
    def delete(self, total_id):
        ...
        # 削除したエントリの参照を返す
        c = self._foods_by_group[group_number][id_in_group]
        del self._foods_by_group[group_number][id_in_group]
        return c
